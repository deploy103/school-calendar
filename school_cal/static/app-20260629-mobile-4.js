...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    cell.tabIndex = 0;
    cell.setAttribute("aria-label", `${formatDateLabel(dateText)} 일정 ${dayEvents.length}개 보기`);
    ...
    const wideHiddenCount = Math.max(dayEvents.length - 4, 0);
    ...
    const narrowHiddenCount = Math.max(dayEvents.length - 3, 0);
